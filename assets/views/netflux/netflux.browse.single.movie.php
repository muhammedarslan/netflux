...
<div class="detail-modal-wrapper">
    ...
    <div class="content">
        <div class="image" style="background: url(<?= json_decode($LastVideo['video_images'], true)[1] ?>">
            <div class="name-area">
                <?= StaticFunctions::VideoTranslation($LastVideo['video_name'], $LastVideo['video_translations'], 'name') ?>
            </div>
            <div class="buttons">
            ...
            <div class="info-area">
                <div>
                    <div class="badges">
                        ...
                        $PopularityOrder = $PopularityClass->SingleItemPopularity($LastVideo['id']);

                        if ($PopularityOrder > 0) {
                            echo '<div class="top">
                        ' . StaticFunctions::lang('541_number-0-this-month-in', [$PopularityOrder]) . '
                        <div class="icon"></div>
                    </div>';
                        }
            ...
            <div class="about">
                <div class="hero">
                    <?= Staticfunctions::lang('547_about', [
                        '<strong>' . StaticFunctions::VideoTranslation($LastVideo['video_name'], $LastVideo['video_translations'], 'name') . '</strong>'
                    ]) ?>
                </div>
                <div class="cast">
                    <p>
                        <span><?= Staticfunctions::lang('549_tracking') ?></span>
                        <?php

                        Jenssegers\Date\Date::setLocale(mb_strtolower(LANG));
                        $date = new  Jenssegers\Date\Date('+' . $LastVideo['video_duration'] . ' second');
                        $DurationText =  Jenssegers\Date\Date::now()->timespan($date);

                        ?>
                        <a href="javascript:;"><?= $DurationText ?></a>
                    </p>
                    <p>
