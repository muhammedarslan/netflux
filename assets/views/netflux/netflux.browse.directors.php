    ...
    $SingleData = $db->query("SELECT * FROM directors WHERE id = '{$ID}'")->fetch(PDO::FETCH_ASSOC);
    if (!$SingleData) {
        StaticFunctions::NoBarba();
        StaticFunctions::go_home();
    }
    ...
    <section class="entity-showcase">
        ...
        <div style="padding-top: 80px;" class="entity-carousel">
            <div style="font-weight:normal;" class="carousel-title">
                <?= StaticFunctions::lang('353_director') . ' ' . StaticFunctions::lang($SingleData['director_name']) ?>
            </div>
        </div>



        <div data-source="list/directors" data-send="<?= $ID ?>" data-title="null" class="entity-carousel list_videos">
