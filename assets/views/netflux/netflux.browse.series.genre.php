    ...
    $ItemCount = $db->query("SELECT id from series_and_movies WHERE video_type='episode' and video_categories LIKE '%\"" . $ID . "\"%' and video_source != '' ", PDO::FETCH_ASSOC)->rowCount();
    if ($ItemCount < 1) {
        StaticFunctions::NoBarba();
        StaticFunctions::go_home();
    }
    ...
    $LastVideo = $db->query("SELECT * FROM series_and_movies WHERE '{$ProfileLevel}' <= video_level and video_type='episode' and video_categories LIKE '%\"" . $ID . "\"%' order by Rand() DESC LIMIT 1 ")->fetch(PDO::FETCH_ASSOC);
    if (!$LastVideo) {
        StaticFunctions::go_home();
    }
...
.list-carousel:hover {
 ...
 <img style="max-width: 650px;max-height:230px;" class="entity-logo mb-4"
    ...
    line-height: 80px;"><?= StaticFunctions::VideoTranslation(@$SeriesVideo['video_name'], @$SeriesVideo['video_translations'], 'name') ?></span>
            ...
            <div data-single="row" data-hide="null" data-send="<?= $SingleGenre['id'] ?>"
                data-source="list/populer/series/genre"
    ...
    <section style="display: flow-root;width:100%;" class="entity-showcase">
        ...
        <div data-source="list/genres/series" data-send="<?= $SingleGenre['id'] ?>" data-title="null"
