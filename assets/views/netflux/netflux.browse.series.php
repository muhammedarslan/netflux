    ...
    $LastVideo = $db->query("SELECT * FROM series_and_movies WHERE '{$ProfileLevel}' <= video_level and video_type='episode' order by Rand() DESC LIMIT 1 ")->fetch(PDO::FETCH_ASSOC);
    if (!$LastVideo) {
    ...
    $SeriesVideo = $db->query("SELECT video_name,id,video_translations,video_level FROM series_and_movies WHERE id='{$SeriesID}'  ")->fetch(PDO::FETCH_ASSOC);
    if (!$SeriesVideo) {
        StaticFunctions::go_home();
    }
...
.category-selector .dropdown ul {
    ...
    white-space: nowrap;" class="carousel-title">
            <?= StaticFunctions::lang('109_series') ?>
        </div>
        <div class="category-selector">
            <div class="toggle">
                <?= Staticfunctions::lang('540_species') ?>
            ...
            <div class="dropdown">
                ...
                if ($GetGenres->rowCount()) {
                    foreach ($GetGenres as $key => $Genre) {
                        $GenreID = $Genre['id'];
                        $ItemCount = $db->query("SELECT id from series_and_movies WHERE '{$ProfileLevel}' <= video_level and video_type='episode' and video_categories LIKE '%\"" . $GenreID . "\"%' and video_source != '' ", PDO::FETCH_ASSOC)->rowCount();
                        if ($ItemCount > 0) {
                ...
                foreach ($GenresObject as $key => $object) {
                    ...
                    foreach ($object as $key => $single) {
                        echo '<li>
                        <a href="' . PATH . 'browse/series/87654' . $single['genreID'] . '">' . $single['genreName'] . '</a>
                    </li>';
                    }
 ...
 <img style="max-width: 650px;max-height:230px;" class="entity-logo mb-4"
    ...
    font-weight: 600;
    font-size: 60px;
    line-height: 80px;"><?= StaticFunctions::VideoTranslation(@$SeriesVideo['video_name'], @$SeriesVideo['video_translations'], 'name') ?></span>
                    <h2 class="mb-4">
                    ...
                    <div>
                        <a
                            href="<?= PATH ?>watch/87654<?= @$SeriesVideo['id'] . '/' . StaticFunctions::BrowseReferer() ?>">
                        ...
                        <a href="<?= PATH ?>browse/87654<?= @$SeriesVideo['id'] ?>">
            ...
            $VideoLevel = StaticFunctions::VideoAdulthoodLevel(@$SeriesVideo['video_level']);
            ...
            <div data-single="row" data-hide="null" data-source="list/populer/series"
    ...
    <section class="entity-showcase">
        ...
        if ($ProfileLevel == 5) {
            ...
            if ($KidsGenre) {
                echo '<div data-source="list/genres/series" data-single="row" data-hide="null" data-send="' . $KidsGenre['id'] . '" data-title="' . StaticFunctions::GenreTranslation($KidsGenre['genres_name'], $KidsGenre['genre_translations']) . '" class="entity-carousel list_videos list-container list-container-relative">
        ...
        if ($Genres->rowCount()) {
            foreach ($Genres as $row) {
                echo '<div data-source="list/genres/series" data-single="row" data-hide="null" data-send="' . $row['id'] . '" data-title="' . StaticFunctions::GenreTranslation($row['genres_name'], $row['genre_translations']) . '" class="entity-carousel list_videos list-container list-container-relative">
