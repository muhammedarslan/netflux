    ...
    $SingleData = $db->query("SELECT * FROM actors WHERE id = '{$ID}'")->fetch(PDO::FETCH_ASSOC);
    if (!$SingleData) {
        StaticFunctions::NoBarba();
        StaticFunctions::go_home();
    }
    ...
    <section class="entity-showcase">
        ...
        <div style="padding-top: 80px;" class="entity-carousel">
            <div style="font-weight:normal;" class="carousel-title">
                <?= StaticFunctions::lang('352_actor') . ' ' . StaticFunctions::lang($SingleData['actor_name']) ?>
            </div>
        </div>



        <div data-source="list/actors" data-send="<?= $ID ?>" data-title="null" class="entity-carousel list_videos">
