...
if (isset($_SESSION['CheckSession']) && $_SESSION['CheckSession'] == 'active') {
    ...
    if ($Email == '' || $Password == '' || !filter_var($Email, FILTER_VALIDATE_EMAIL)) {
        echo StaticFunctions::JsonOutput([
            'status' => 'failed',
            'label' => 'info',
            'message' => StaticFunctions::lang('71_we-could-not-find-an-administrator')
        ]);
        exit;
    } else {

        $LoginType = 'Login';
        $UserQuery = $db->query("SELECT * FROM users WHERE user_type='admin'  and  email='{$Email}' ")->fetch(PDO::FETCH_ASSOC);
