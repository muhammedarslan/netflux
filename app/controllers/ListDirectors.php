...
$DirectorID = StaticFunctions::post('data');
...
$Search = $db->query("SELECT * FROM series_and_movies
    WHERE video_type='movie' and video_directors LIKE '%\"{$DirectorID}\"%' LIMIT 20
    ", PDO::FETCH_ASSOC);
if ($Search->rowCount()) {
...
$Search = $db->query("SELECT * FROM series_and_movies
    WHERE video_type='series' and video_directors LIKE '%\"{$DirectorID}\"%' LIMIT 20
    ", PDO::FETCH_ASSOC);
if ($Search->rowCount()) {
    foreach ($Search as $row) {
        array_push($ListItems2, $row);
...
$Search = $db->query("SELECT * FROM series_and_movies
    WHERE video_type='episode' and video_directors LIKE '%\"{$DirectorID}\"%' LIMIT 20
    ", PDO::FETCH_ASSOC);
if ($Search->rowCount()) {
    foreach ($Search as $row) {
        array_push($ListItems2, $row);
